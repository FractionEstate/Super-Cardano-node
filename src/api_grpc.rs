...
impl chaindb_proto::chain_db_service_server::ChainDbService for ChainDbGrpcServer {
    type StreamBlocksStream =
        Pin<Box<dyn Stream<Item = Result<chaindb_proto::Block, Status>> + Send + 'static>>;
    type StreamUTXOsStream =
        Pin<Box<dyn Stream<Item = Result<chaindb_proto::UtxoEntry, Status>> + Send + 'static>>;

    async fn get_block(
        &self,
        request: Request<chaindb_proto::GetBlockRequest>,
    ) -> Result<Response<chaindb_proto::Block>, Status> {
    ...
    async fn get_utxo(
        &self,
        request: Request<chaindb_proto::GetUtxoRequest>,
    ) -> Result<Response<chaindb_proto::UtxoResponse>, Status> {
        let req = request.into_inner();
        let db = self.db.read().await;
        let utxo = db
            .api_get_utxo(req.block_id, req.tx_id, req.index)
            .await
            .map_err(|e| Status::not_found(e.to_string()))?;
        Ok(Response::new(chaindb_proto::UtxoResponse {
            output: utxo.as_ref().map(|o| txoutput_to_proto(o)),
        }))
    }

    async fn stream_blocks(
        &self,
        _request: Request<chaindb_proto::StreamBlocksRequest>,
    ) -> Result<
        Response<
            <Self as chaindb_proto::chain_db_service_server::ChainDbService>::StreamBlocksStream,
        >,
        Status,
    > {
        let blocks: Vec<_> = {
    ...
    async fn stream_utx_os(
        &self,
        request: Request<chaindb_proto::StreamUtxOsRequest>,
    ) -> Result<
        Response<
            <Self as chaindb_proto::chain_db_service_server::ChainDbService>::StreamUTXOsStream,
        >,
        Status,
    > {
        let utxos: Vec<_> = {
            ...
            let db = self.db.read().await;
            let mut stream = db
                .api_stream_utxos(block_id)
                .await
                .map_err(|e| Status::internal(e.to_string()))?;
            let mut utxos = Vec::new();
            use futures::StreamExt;
            while let Some(((tx_id, index), output)) = stream.next().await {
                utxos.push(chaindb_proto::UtxoEntry {
                    tx_id,
                    index,
                    output: Some(txoutput_to_proto(&output)),
                });
            }
            utxos
        };
...
pub async fn start_grpc_server(
    shared_db: crate::chaindb::SharedChainDB,
